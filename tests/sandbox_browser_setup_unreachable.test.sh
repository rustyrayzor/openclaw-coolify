...
bash <(tr -d '\r' < "$ROOT_DIR/scripts/sandbox-browser-setup.sh") >"$OUTPUT_FILE" 2>&1
STATUS=$?
set -e

if [[ $STATUS -ne 0 ]]; then
  echo "Expected sandbox-browser-setup.sh to continue when Docker is unreachable."
  cat "$OUTPUT_FILE"
  exit 1
fi
...
echo "PASS: sandbox-browser-setup.sh skips prebuild when Docker is unreachable."
